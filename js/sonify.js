...
(function (global) {
  ...
  // ★注意：これは「収録全体」に掛けること。区間を切ってから掛けてはいけない。
  //   位相＝一周のどこにいるかは、その瞬間だけでは決まらず前後の波を見て決まる。
  //   区間の頭には「前」が無いので、そこだけ計算が捏造になる。
  //   実測での差は位相の中央値で数度（区間が短いほど大きく、1〜2秒で5〜6°、10秒超で2°以下）。
  //   大きくはないが、なぞりの起点＝音の0秒＝紫の瞬間がそこに当たるので残す理由が無い。
  //   呼び出し側で収録全体に掛けて、あとから区間を切ること。
  function analytic(gyro, fs, cutoff) {
    let sig = new Float64Array(gyro.length);
    for (let i = 0; i < gyro.length; i++) sig[i] = gyro[i] * 2000;
    // ★平均を引く（Lamb & Stöckl 2014 が推奨する手順の1番目）。
    //   ジャイロにはゼロ点のずれ（バイアス）が乗る。ローパスはDCを通すので、
    //   引かないとバイアスがそのまま入り、位相面が原点からずれて偏角が歪む。
    //   ⚠️振幅の正規化はしない。彼らはそれが周波数アーティファクトを生むとして退けている。
    let mean = 0;
